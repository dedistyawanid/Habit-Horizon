 ...
 * Uses dynamic user ID from supabase.ts (set by AuthContext).
 */
...
export function syncHabit(h: { id: string; name: string; category?: string; frequency?: string; color?: string; icon?: string; createdAt?: string; weeklyStreakTarget?: number; targetDays?: number[] }) {
  return upsert("habits", { id: h.id, user_id: getUserId(), name: h.name, category: h.category ?? null, frequency: h.frequency ?? "Daily", color: h.color ?? null, icon: h.icon ?? null, created_at: h.createdAt ?? new Date().toISOString(), weekly_streak_target: h.weeklyStreakTarget ?? null, target_days: h.targetDays ? JSON.stringify(h.targetDays) : null });
}
...
export function syncCheckIn(c: { id: string; habitId: string; date: string; notes?: string; completedAt?: string }) {
  return upsert("checkins", { id: c.id, user_id: getUserId(), habit_id: c.habitId, date: c.date, notes: c.notes ?? null, completed_at: c.completedAt ?? new Date().toISOString() });
}
...
export function syncActivity(e: { id: string; date: string; type: string; durationMin?: number; distanceKm?: number; elevationGain?: number; runType?: string; notes?: string; createdAt?: string }) {
  return upsert("activity_logs", { id: e.id, user_id: getUserId(), date: e.date, type: e.type, duration_min: e.durationMin ?? null, distance_km: e.distanceKm ?? null, elevation_gain: e.elevationGain ?? null, run_type: e.runType ?? null, notes: e.notes ?? null, created_at: e.createdAt ?? new Date().toISOString() });
}
...
export function syncMeal(e: { id: string; date: string; name: string; calories: number; protein: number; carbs?: number; createdAt?: string }) {
  return upsert("nutrition_logs", { id: e.id, user_id: getUserId(), date: e.date, name: e.name, calories: e.calories, protein: e.protein, carbs: e.carbs ?? 0, created_at: e.createdAt ?? new Date().toISOString() });
}
...
export function syncSleep(e: { id: string; date: string; hours: number; minutes: number; quality: number; createdAt?: string }) {
  return upsert("sleep_logs", { id: e.id, user_id: getUserId(), date: e.date, hours: e.hours, minutes: e.minutes, quality: e.quality, created_at: e.createdAt ?? new Date().toISOString() });
}
...
export function syncWeight(e: { id: string; date: string; weight: number }) {
  return upsert("weight_logs", { id: e.id, user_id: getUserId(), date: e.date, weight: e.weight, created_at: new Date().toISOString() });
}
...
export function syncTransaction(t: { id: string; type: string; amount: number; category?: string; date: string; title?: string; note?: string; notes?: string; createdAt?: string }) {
  return upsert("finance_logs", { id: t.id, user_id: getUserId(), type: t.type, amount: t.amount, category: t.category ?? null, date: t.date, note: t.note ?? t.notes ?? t.title ?? null, created_at: t.createdAt ?? new Date().toISOString() });
}
...
export function syncNote(n: { id: string; title: string; content?: string; category?: string; createdAt?: string; updatedAt?: string }) {
  return upsert("notes", { id: n.id, user_id: getUserId(), title: n.title, content: n.content ?? null, category: n.category ?? null, created_at: n.createdAt ?? new Date().toISOString(), updated_at: n.updatedAt ?? new Date().toISOString() });
}
